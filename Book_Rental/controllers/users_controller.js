...
module.exports.returnred =function(req,res){
    ...
    exec(function(err,book){
        ...
        if( OTP==book.OTP){
            ...
            Books.findByIdAndUpdate(bookid, { returned: true, returndate: utc, total: tprice}, {upsert: true, useFindAndModify: false}, function(err1){
                if(err1){
                    console.log('Error updating data');
                }
...
module.exports.update_data =async function(req,res){
    ...
    if(req.user.password != req.body.pwd){
        Users.update({_id:req.user.id}, {$set: { name:req.body.name, email: req.body.email, username: req.body.username, phno:req.body.phno, address:req.body.address}}, {upsert: true}, function(err){
            if(err){
